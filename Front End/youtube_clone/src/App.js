...
class App extends Component {
    ...
    getComments = async () => {
        try{
            let response =await axios.get(`http://127.0.0.1:8000/comments/${this.state.videoId}`);
            this.setState({
    ...
    render() { 
        return ( 
        <React.Fragment>
            <SearchBar getVideo={this.getVideo}  data={this.state.data} /><br />
            <MainVideo data={this.state.data} videoId={this.state.videoId} videoInfo={this.state.videoInfo} /> 
            <Results selectVideo={this.selectVideo} data={this.state.data} />
            <Comments getComments={this.getComments} comments={this.state.comments} videoID={this.state.videoId}/>
        </React.Fragment>
        );
    }
}
